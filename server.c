...
/*   Updated: 2023/04/14 13:50:29 by atucci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static int	current_char_bits; // Stores the bits of the current character being received
	static int	current_bit_index; // Stores the index of the bit currently being received

	(void)unused; // Unused parameter

	// If the signal received is SIGUSR1, set the corresponding bit in the current character bits
	if (signal_number == SIGUSR1)
		current_char_bits += 1 << current_bit_index; // 1 << current_bit_index is equivalent to 2^current_bit_index

	current_bit_index++; // Move to the next bit

	// If all 8 bits of the current character have been received, print the character and reset the bit counters
	if (current_bit_index == 8)
	{
		...
		{	
			usleep(1); // wait for a short delay GPT
